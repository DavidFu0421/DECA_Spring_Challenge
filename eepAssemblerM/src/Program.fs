...
let opMap: Map<string,Token> = 
    Map.ofList [
        ...
        "MOVC1.1", MOVEXTRA 9 // Booth MUL
        "MOVC2.1", MOVEXTRA 10 // Booth MUL
        "MOVC3.1", MOVEXTRA 11 // Booth MUL
        "MOVC4.1", MOVEXTRA 12 // Booth MUL --> get lower 16 bits
        "MOVC4.2", MOVEXTRA 13 // Booth MUL --> get upper 16 bits
        "MUL", MOVEXTRA 1 // get low
        "MULH", MOVEXTRA 2 // get high by doing everything again
        "MUL_H", MOVEXTRA 3 // get high after MUL
        "FMUL.H", MOVEXTRA 4 // half precision floating point multiply
        "ADD", ALUOP 1
        "SUB", ALUOP 2
        "ADC", ALUOP 3
        "SBC", ALUOP 4
...
let makeMovExtraOp ra rb x =
    fun symTab ->
        ...
        |> Result.bind (fun (a,b) -> 
            match x with
            | 9 -> Ok (IWord.AluOpcField(0) ||| IWord.RaField a ||| IWord.RbField b ||| IWord.RcField 1 ||| 0x1u)
            | 10 -> Ok (IWord.AluOpcField(0) ||| IWord.RaField a ||| IWord.RbField b ||| IWord.RcField 2 ||| 0x1u)
            | 11 -> Ok (IWord.AluOpcField(0) ||| IWord.RaField a ||| IWord.RbField b ||| IWord.RcField 3 ||| 0x1u)
            | 12 -> Ok (IWord.AluOpcField(0) ||| IWord.RaField a ||| IWord.RbField b ||| IWord.RcField 4 ||| 0x1u)
            | 13 -> Ok (IWord.AluOpcField(0) ||| IWord.RaField a ||| IWord.RbField b ||| IWord.RcField 4 ||| 0x2u)
            | _ -> Ok (IWord.AluOpcField(0) ||| IWord.RaField a ||| IWord.RbField b ||| IWord.RcField x))
